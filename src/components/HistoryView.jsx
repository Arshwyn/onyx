...
export default function HistoryView() {
  ...
  useEffect(() => {
    ...
    const loadSettings = () => {
      ...
      setDistUnit((localStorage.getItem('onyx_unit_distance') || 'mi').toLowerCase()); // NEW
    };
    loadSettings();
  ...
  // ... (Keep existing loadData, handleDelete, startEditing, saveEdit, updateEditSet etc.) ...
              ...
              <div key={`c-${item.id}`} className="bg-zinc-900/80 rounded border border-blue-900/30 overflow-hidden relative group">
                <div className="p-3 flex justify-between items-center">
                  <div className="flex items-center gap-3 min-w-0 flex-1">
                    <div className="bg-blue-900/20 text-blue-400 p-2 rounded-full flex-shrink-0">
                      {/* REPLACED 'C' WITH HEART ICON */}
                      <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M4.318 6.318a4.5 4.5 0 000 6.364L12 20.364l7.682-7.682a4.5 4.5 0 00-6.364-6.364L12 7.636l-1.318-1.318a4.5 4.5 0 00-6.364 0z"></path></svg>
                    </div>
                    <div className="min-w-0"><span className="text-blue-400 font-bold block truncate">{item.type}</span><span className="text-xs text-zinc-500 font-mono">{item.date}</span></div>
                  </div>
          ...
          return (
            <div key={`l-${item.id}`} className="bg-zinc-900 rounded border border-zinc-800 overflow-hidden relative group">
              <div className="bg-zinc-800/50 p-3 flex justify-between items-center">
                <div className="min-w-0 flex-1 pr-2"><span className="font-bold text-gray-200 block truncate">{exerciseMap[item.exerciseId || item.exercise_id] || 'Unknown Exercise'}</span><span className="text-xs text-zinc-500 font-mono">{item.date}</span></div>
                <div className="flex gap-2 flex-shrink-0"><button onClick={() => startEditing(item)} className="text-xs bg-zinc-700 hover:bg-zinc-600 text-white px-2 py-1 rounded transition">Edit</button><button onClick={() => handleDelete(item)} className="text-xs bg-red-900/30 hover:bg-red-900/50 text-red-400 px-2 py-1 rounded border border-red-900/50 transition">Del</button></div>
              </div>
              <div className="p-3">
