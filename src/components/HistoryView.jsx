...
export default function HistoryView() {
  ...
  const loadData = () => {
    ...
    const loadedCardio = getCardioLogs();
  ...
  const downloadCSV = () => {
    ...
    combinedHistory.forEach(item => {
      if (item.dataType === 'lift') {
        const exName = exerciseMap[item.exerciseId] || 'Unknown';
  ...
  return (
    <div className="max-w-md mx-auto text-white pb-10">
              ...
              <div key={`c-${item.id}`} className="bg-zinc-900/80 rounded border border-blue-900/30 overflow-hidden relative group">
                <div className="p-3 flex justify-between items-center">
                  <div className="flex items-center gap-3">
                     ...
                     <div>
                       <span className="text-blue-400 font-bold block">{item.type}</span>
                  ...
                  <div className="flex items-center gap-4">
                    ...
                    {/* NEW: Matching Buttons */}
                    <div className="flex gap-2">
                        <button onClick={() => startEditing(item)} className="text-xs bg-zinc-800 hover:bg-zinc-700 text-white px-2 py-1 rounded transition border border-zinc-700">
                            Edit
                        </button>
                        <button onClick={() => handleDelete(item)} className="text-xs bg-red-900/20 hover:bg-red-900/40 text-red-400 px-2 py-1 rounded border border-red-900/50 transition">
                            Del
                        </button>
                    </div>
                  </div>
          ...
          return (
            <div key={`l-${item.id}`} className="bg-zinc-900 rounded border border-zinc-800 overflow-hidden relative group">
              <div className="bg-zinc-800/50 p-3 flex justify-between items-center">
                ...
                <div className="flex gap-2">
                  <button onClick={() => startEditing(item)} className="text-xs bg-zinc-700 hover:bg-zinc-600 text-white px-2 py-1 rounded transition">
                    Edit
                  </button>
                  <button onClick={() => handleDelete(item)} className="text-xs bg-red-900/30 hover:bg-red-900/50 text-red-400 px-2 py-1 rounded border border-red-900/50 transition">
                    Del
                  </button>
                </div>
              </div>
      ...
      {/* --- EDIT MODAL (Dynamic for Lift or Cardio) --- */}
      {editingLog && (
        <div className="fixed inset-0 bg-black/80 backdrop-blur-sm flex items-center justify-center p-4 z-50">
          <div className="bg-zinc-900 w-full max-w-sm rounded-xl border border-zinc-700 shadow-2xl p-4 animate-fade-in">
            <h3 className="text-lg font-bold mb-4 text-white">
                Edit {editingLog.dataType === 'lift' ? 'Workout' : 'Cardio'}
            </h3>
                    ...
                    <div className="flex gap-3">
                        ...
                        <div className="flex-1">
                            ...
                            <input 
                                ...
                                onChange={(e) => setEditingLog({ ...editingLog, distance: e.target.value })}
                                className="w-full bg-black border border-zinc-700 rounded p-2 text-white outline-none"
                            />
                        </div>
                    </div>
