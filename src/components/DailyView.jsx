...
export default function DailyView() {
  ...
  const [personalRecords, setPersonalRecords] = useState({}); // NEW: Store PRs
  ...
  const loadView = async (targetDate, showLoading = true) => {
    try {
        ...
        if (routine) {
            ...
            // 1. CALCULATE LAST PERFORMANCE (Previous Session)
            const historyStats = {};
            mergedData.forEach(ex => {
            ...
            // 2. CALCULATE PERSONAL RECORDS (All Time Max)
            const prMap = {};
            mergedData.forEach(ex => {
                // Filter all logs for this exercise, excluding today to compare fairly
                const pastLogs = allLogs.filter(l => 
                    String(l.exercise_id || l.exerciseId) === String(ex.id) && 
                    new Date(l.date) < new Date(dateStr) 
                );
  ...
  const handleSwapToToday = () => {
    ...
    openConfirm('Swap Routine?', `Do you want to replace today's workout with ${currentRoutine.name}?`, () => {
        const now = new Date();
  ...
  const handleRevertSchedule = () => {
    openConfirm('Revert Schedule?', 'This will go back to the original scheduled routine for today.', () => {
  ...
  const handleToggleAdHocRest = () => {
    ...
    if (newState) { localStorage.setItem(`onyx_rest_${dateStr}`, 'true'); } 
    else { localStorage.removeItem(`onyx_rest_${dateStr}`); }
  };

  // --- BODY STATS & CARDIO ACTIONS ---
  ...
  const handleSaveCardio = async () => { if (!cardioDuration) { openConfirm("Missing Duration", "Please enter a duration in minutes."); return; } const dateStr = getDateStr(viewDate); const dist = cardioDistance === '' ? null : cardioDistance; const newLogs = await addCardioLog(dateStr, cardioType, cardioDuration, dist); const todaysCardio = newLogs.filter(c => c.date === dateStr); setViewCardioLogs(todaysCardio); setCardioDuration(''); setCardioDistance(''); setShowCardioForm(false); };
      ...
      {isAdHocRest ? (<div className="flex flex-col items-center justify-center py-20 bg-zinc-900/50 rounded-xl border border-zinc-800"><div className="w-16 h-16 bg-zinc-800 rounded-full flex items-center justify-center mb-4"><span className="text-3xl">☕</span></div><h2 className="text-2xl font-black italic uppercase text-white mb-2">Taking it Easy</h2><p className="text-zinc-500 text-sm mb-6">Recovery is when the growth happens.</p><button onClick={handleToggleAdHocRest} className="text-xs text-zinc-600 underline hover:text-white">No, I actually want to workout</button></div>) : isScheduledRest ? (<div className="flex flex-col items-center justify-center py-20 bg-zinc-900/50 rounded-xl border border-zinc-800"><div className="w-16 h-16 bg-zinc-800 rounded-full flex items-center justify-center mb-4"><span className="text-3xl">☕</span></div><h2 className="text-2xl font-black italic uppercase text-white mb-2">Scheduled Rest</h2><p className="text-zinc-500 text-sm mb-6">Enjoy your day off.</p></div>) : isNoRoutine ? (<div className="text-center mt-10 text-gray-500"><p>No routine scheduled for {dayName}.</p><p className="text-xs mt-4">Go to "Manage" to set up a routine.</p></div>) : (
        <div className="space-y-4">
          {exercises.map((ex, idx) => {
            ...
            // CHECK IF COMPLETED WEIGHT IS A PR
            // We check the sets input for this exercise
            const currentSets = setInputs[ex.id] || [];
            let maxLifted = 0;
            currentSets.forEach(s => {
                const w = parseFloat(s.weight);
                if (w > maxLifted) maxLifted = w;
            });
              ...
              <div key={ex.id} className={`rounded-lg overflow-hidden transition-all duration-300 border ${isComplete ? 'bg-zinc-900 border-green-900/50' : 'bg-zinc-900 border-zinc-800'}`}>
                <div onClick={() => isComplete && toggleExpand(ex.id)} className={`p-4 flex justify-between items-center ${isComplete ? 'cursor-pointer select-none' : ''}`}>
                  ...
                  <div className="text-right flex flex-col items-end gap-2 ml-2 flex-shrink-0">
                    {!isComplete && (<div className="flex gap-1 mb-1"><button onClick={(e) => moveExercise(idx, -1, e)} className={`w-6 h-6 rounded bg-zinc-800 flex items-center justify-center text-zinc-400 hover:text-white hover:bg-zinc-700 transition ${idx === 0 ? 'opacity-0 pointer-events-none' : ''}`}><svg className="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M5 15l7-7 7 7"></path></svg></button><button onClick={(e) => moveExercise(idx, 1, e)} className={`w-6 h-6 rounded bg-zinc-800 flex items-center justify-center text-zinc-400 hover:text-white hover:bg-zinc-700 transition ${idx === exercises.length - 1 ? 'opacity-0 pointer-events-none' : ''}`}><svg className="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M19 9l-7 7-7-7"></path></svg></button></div>)}
                    {isComplete ? (<div className="text-zinc-500 text-xs font-bold uppercase tracking-wider flex items-center gap-1">{isExpanded ? 'Hide' : 'Show'} <span className={`text-lg leading-none transition-transform ${isExpanded ? 'rotate-180' : ''}`}>⌄</span></div>) : (<><div className="mb-1"><span className="text-[10px] text-zinc-500 uppercase font-bold block">GOAL</span><span className="text-sm font-mono text-blue-400 font-bold">{ex.targetSets} x {ex.targetReps}</span></div>{lastStats && <div><span className="text-[10px] text-zinc-500 uppercase font-bold block">LAST</span><span className="text-sm font-mono text-gray-300">{lastStats}</span></div>}</>)}
                  </div>
                </div>
                {showBody && (
                  <div className={isComplete ? "opacity-50" : ""}>
                    <div className="px-4 pb-4 space-y-3">
                      ...
                      {setInputs[ex.id]?.map((set, idx) => (
                        <div key={idx} className="flex gap-3 items-center"><div className="w-8 text-center text-zinc-600 font-bold text-sm">{idx + 1}</div><div className="flex-1 relative"><input type="number" placeholder="-" value={set.weight} onChange={(e) => handleSetChange(ex.id, idx, 'weight', e.target.value)} className="w-full bg-black border border-zinc-700 rounded p-2 text-white text-center outline-none focus:border-blue-500 transition font-mono" /><button onClick={() => openCalculator(set.weight)} className="absolute right-1 top-1/2 -translate-y-1/2 text-zinc-600 hover:text-blue-400 p-1"><svg className="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M9 7h6m0 10v-3m-3 3h.01M9 17h.01M9 14h.01M12 14h.01M15 11h.01M12 11h.01M9 11h.01M7 21h10a2 2 0 002-2V5a2 2 0 00-2-2H7a2 2 0 00-2 2v14a2 2 0 002 2z"></path></svg></button></div><div className="flex-1"><input type="number" placeholder="-" value={set.reps} onChange={(e) => handleSetChange(ex.id, idx, 'reps', e.target.value)} className="w-full bg-black border border-zinc-700 rounded p-2 text-white text-center outline-none focus:border-blue-500 transition font-mono" /></div></div>
                      ))}
                    </div>
                    <div className="p-3 bg-zinc-800/20 border-t border-zinc-800"><button onClick={() => handleLogExercise(ex.id)} className="w-full bg-white text-black font-bold py-3 rounded hover:bg-gray-200 transition tracking-widest text-xs uppercase">{isComplete ? 'Update Log' : 'Complete Exercise'}</button></div>
                  </div>
                )}
              </div>
            );
