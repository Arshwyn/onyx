...
export default function WorkoutLogger() {
  ...
  const [distUnit, setDistUnit] = useState('mi'); // NEW
  ...
  const handleRemoveSet = (index) => { const setToRemove = sets[index]; const hasData = setToRemove.weight || setToRemove.reps; const remove = () => { const newSets = sets.filter((_, i) => i !== index); setSets(newSets); }; if (hasData) { openConfirm("Remove Set?", "This set has data. Are you sure you want to remove it?", remove, true); } else { remove(); } };
  const saveLift = async () => { const validSets = sets.filter(s => s.weight && s.reps); if (validSets.length === 0) { openConfirm("Missing Data", "Please enter weight and reps for at least one set."); return; } setIsSubmitting(true); try { await addLog(date, exerciseId, validSets); openConfirm("Workout Logged", "Great work! Your session has been saved to history.", () => { setSets([{ weight: '', reps: '' }]); }, false); } catch (err) { openConfirm("Error", "Could not save workout. Please try again.", null, true); } finally { setIsSubmitting(false); } };
    ...
    <div className="w-full max-w-md mx-auto text-white overflow-x-hidden">
      ...
      {mode === 'lifting' && (
        <div className="bg-zinc-900 p-4 rounded-lg border border-zinc-800">
          <div className="mb-4"><label className="block text-xs text-gray-500 mb-1 uppercase font-bold">Date</label><input type="date" value={date} onChange={(e) => setDate(e.target.value)} className="w-full bg-black border border-zinc-700 rounded p-2 text-white" /></div>
          <div className="mb-4"><label className="block text-xs text-gray-500 mb-1 uppercase font-bold">Exercise</label><select value={exerciseId} onChange={(e) => setExerciseId(e.target.value)} className="w-full bg-black border border-zinc-700 rounded p-2 text-white outline-none">{exercises.map(ex => (<option key={ex.id} value={ex.id}>{ex.name}</option>))}</select></div>
          <div className="space-y-2 mb-4"><label className="block text-xs text-gray-500 mb-1 uppercase font-bold">Sets</label>{sets.map((set, index) => (<div key={index} className="flex gap-2"><div className="flex-1 relative"><input type="number" placeholder={weightUnit} value={set.weight} onChange={(e) => handleSetChange(index, 'weight', e.target.value)} className="w-full bg-black border border-zinc-700 rounded p-2 text-white" /><button onClick={() => openCalculator(set.weight)} className="absolute right-1 top-1/2 -translate-y-1/2 text-zinc-600 hover:text-blue-400 p-1"><svg className="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M9 7h6m0 10v-3m-3 3h.01M9 17h.01M9 14h.01M12 14h.01M15 11h.01M12 11h.01M9 11h.01M7 21h10a2 2 0 002-2V5a2 2 0 00-2-2H7a2 2 0 00-2 2v14a2 2 0 002 2z"></path></svg></button></div><div className="flex-1"><input type="number" placeholder="reps" value={set.reps} onChange={(e) => handleSetChange(index, 'reps', e.target.value)} className="w-full bg-black border border-zinc-700 rounded p-2 text-white" /></div><button onClick={() => handleRemoveSet(index)} className="text-red-500 px-2">✕</button></div>))}<button onClick={handleAddSet} className="text-xs text-blue-400 hover:text-blue-300 font-bold">+ Add Set</button></div>
      ...
      {mode === 'cardio' && (
        <div className="bg-zinc-900 p-4 rounded-lg border border-zinc-800 animate-fade-in">
          <div className="mb-4"><label className="block text-xs text-gray-500 mb-1 uppercase font-bold">Date</label><input type="date" value={date} onChange={(e) => setDate(e.target.value)} className="w-full bg-black border border-zinc-700 rounded p-2 text-white" /></div>
          <div className="mb-4"><label className="text-[10px] text-zinc-500 uppercase font-bold block mb-1">Type</label><select value={cardioType} onChange={(e) => setCardioType(e.target.value)} className="w-full bg-black border border-zinc-700 rounded p-2 text-white outline-none">{CARDIO_TYPES.map(t => <option key={t} value={t}>{t}</option>)}</select></div>
